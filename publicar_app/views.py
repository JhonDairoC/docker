...
class UpdateProduct(APIView):
    def put(self, request, product_id):
        try:
            ...
            quantity = data.get('orden')
            
            # Validar si el producto existe en la base de datos
            product_ref = db.collection('produc').document(product_id).get()
            ...
            product_data['peso'] = quantity
            
            # Guardar los cambios en la base de datos
            db.collection('produc').document(product_id).update(product_data)
            
            return JsonResponse({'message': 'Cantidad del producto actualizada correctamente'}, status=200)
